...
namespace ClassLibraryHelper.Encrypt.RSA
{
    /// <summary>
    /// 非对称RSA加密类 可以参考
    ...
    /// 若是私匙加密 则需公钥解密
    /// 反正公钥加密 私匙来解密
    /// 需要BigInteger类来辅助,解决超过128位秘钥的导致的问题。
    /// </summary>
    public static class RSAHelper
    {
        ...
        private static string EncryptString(string source, BigInteger d, BigInteger n)
        {
            ...
            for (int i = 0; i < len1; i++)
            {
                ...
                byte[] oText = global::System.Text.Encoding.Default.GetBytes(block);
        ...
        private static string DecryptString(string encryptString, BigInteger e, BigInteger n)
        {
            ...
            for (int i = 0; i < strarr1.Length; i++)
            {
                ...
                string temp = global::System.Text.Encoding.Default.GetString(biEnText.getBytes());
                result.Append(temp);
            }
